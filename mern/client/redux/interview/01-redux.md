...
ধরা যাক আমাদের view তে আছে: `- 0 +`

- এখানে `0` হলো state, যেটা store এ আছে। আমাদের এই view টা store এর সাথে connected — একে বলে **subscribe** করা (অনেকটা "listen" করে থাকার মতো)।
- `+` বাটনে ক্লিক করলে একটি **event** trigger হয়।
- Event handler এর মধ্যে আমরা একটি action **dispatch** করি — এখানে reducer কে জানানো হয় যে increment করতে হবে।
- Action এর মধ্যে দুটি জিনিস থাকে — **type** (যেমন: `increment`)।
- এই action type নিয়ে reducer এর কাছে যায়। Reducer বুঝে যায় কোন লজিক execute করতে হবে (এক্ষেত্রে increment এর লজিক)।
- Logic execute করার সময় reducer এর state দরকার হয় — আগের state ছিল `0`, নতুন state হয় `1`।
- এই নতুন state (`1`) store আপডেট করে view তে দেখায়।

এটাই পুরো Redux সাইকেল।
